...
var et_blockVV  = false;
var et_blockFB  = false;
var et_vvHost	= ('https:' == document.location.protocol ? 'https:' : 'http:' ) + '//visitorvoice.etracker.com/';
function _etc_fb_cb(s,p,p_h,u,e,e_h,ocs)
{
	try 
	{
		if(ocs==false)
		{
			...
			{
				var _fb_img = document.createElement('img');
				document.getElementsByTagName('body')[0].appendChild(_fb_img);
				_fb_img.id = "_fb_img";
			}
			else
			{
				document.write('<img id="_fb_img" />');
				var _fb_img = document.getElementById('_fb_img');
			}
			_fb_img.src = (e ? s : (s+'.png'));
			_fb_img.onclick = function() { _etc_fb_col(false,u);}
			_fb_img.style.position = 'fixed';
			if(document.documentElement.clientHeight==0)
			{
				_fb_img.style.position = 'absolute';
			}
			_fb_img.style.cursor   = 'pointer';
			_fb_img.style.zIndex   = '700';
		}
		document.getElementById('_fb_img').onmouseover = function() 
		{ 
			document.getElementById('_fb_img').src 		= (e ? (('https:' == document.location.protocol ? 'https:' : 'http:' ) + '//' + e_h) : (s+'_h.png')); 
			if(p_h[0])
				document.getElementById('_fb_img').style.left 	 = p_h[0];
			if(p_h[1])
				document.getElementById('_fb_img').style.top 	 = p_h[1];
			if(p_h[2])
				document.getElementById('_fb_img').style.bottom = p_h[2];		
			if(p_h[2])
				document.getElementById('_fb_img').style.right  = p_h[3];
				
			document.getElementById('_fb_img').style.width 	 = p_h[4];
			document.getElementById('_fb_img').style.height  = p_h[5];
		}
		document.getElementById('_fb_img').onmouseout  = function() 
		{ 
			document.getElementById('_fb_img').src 		=  (e ? s : s+'.png'); 
			if(p[0])
				document.getElementById('_fb_img').style.left 	 = p[0];
			if(p[1])
				document.getElementById('_fb_img').style.top 	 = p[1];
			if(p[2])
				document.getElementById('_fb_img').style.bottom = p[2];		
			if(p[2])
				document.getElementById('_fb_img').style.right  = p[3];
		}
		...
		document.getElementById('_fb_img').style.border  = 'none';
		document.getElementById('_fb_img').style.width 	 = p[4];
		document.getElementById('_fb_img').style.height  = p[5];
	}catch (e) {}
}
...
{						
	...
	if (self && self.innerHeight)  {
		et_cWidth  = self.innerWidth;
		et_cHeight = self.innerHeight;
	}
	else if (document.documentElement && document.documentElement.clientHeight) {
		et_cWidth  = document.documentElement.clientWidth;
		et_cHeight = document.documentElement.clientHeight;
	}
	else if (document.body) {
		et_cWidth  = document.body.clientWidth;
		et_cHeight = document.body.clientHeight;
	}  
		
	return new Array(et_cWidth, et_cHeight);
}
...
function _etc_fb_get_scroll_sizes() {
	...
	if( typeof( window.pageYOffset ) == 'number' ) {
		//Netscape compliant
		et_scrOfY = window.pageYOffset;
		et_scrOfX = window.pageXOffset;
	} else if( document.body && ( document.body.scrollLeft || document.body.scrollTop ) ) {
		//DOM compliant
		et_scrOfY = document.body.scrollTop;
		et_scrOfX = document.body.scrollLeft;
	} else if( document.documentElement && ( document.documentElement.scrollLeft || document.documentElement.scrollTop ) ) {
		//IE6 standards compliant mode
		et_scrOfY = document.documentElement.scrollTop;
		et_scrOfX = document.documentElement.scrollLeft;
	}
	return new Array(et_scrOfX, et_scrOfY);
}


function _etc_fb_col(l,u)
{
	if(!l) l='';
	...
	{
		document.getElementsByTagName("body")[0].removeChild(document.getElementById('feedbackOverlay'));
		
		if(document.getElementById('feedbackStyleOverlay'))
			document.getElementsByTagName("head")[0].removeChild(document.getElementById('feedbackStyleOverlay'));
		if(document.getElementById('et_vv_fb_ol_div'))
			document.getElementsByTagName("body")[0].removeChild(document.getElementById('et_vv_fb_ol_div'));
	}
	
	var e = document.createElement("script");
	e.src = et_vvHost + 'feedback.php?et=qPKGYV&l=' + l + '&q=2&u=' + u;
	e.type="text/javascript";
	e.id="feedbackOverlay";
   	document.getElementsByTagName("body")[0].appendChild(e);
}

function _etc_fb_ol_close()
{
	document.getElementById('et_vv_fb_content').style.display='none';
	document.getElementById('et_vv_fb_fade').style.display='none';
}

function _etc_fb_sd(send_prms,u)
{
	if(document.getElementById('et_vvfb_q1_v').value == '0')
	{
		document.getElementById('vvfb_q_starscale_error_msg').style.display='block';
		return false;
	}
	
	var prms = '', ar;
	
	if(typeof(et_pagename) != 'undefined' && typeof(et_pagename) != 'unknown' && et_pagename != '')
	{
		prms += "&p=" + escape(et_pagename);
		et_easy = 0;
	}
	else
		et_easy = 1;
	...
	if (typeof(et_url)=='string')
		prms += "&url=" + escape(et_url ? et_url : ar[0]);
	else
		prms += "&url=" + escape(ar[0]);
	
	
	_vv_fb_cnt 	   = new Image(1,1);
	_vv_fb_cnt.src = et_vvHost+'vvcnt.php?et=qPKGYV&t=vfb&u='+u+'&q=2' + prms + '&' + send_prms;
...
function etEvent(host, src, secure) 
{
	var self = this;
	var etHost = host;
	var etSrc = src;
...
// fetch url parameter
var et_server       = ('https:' == document.location.protocol ? 'https:' : 'http:' ) + '//www.etracker.de';
var et_referer      = et_server + '/app?et=';
var et_cntScript    = 'cnt.php';
var et_ver          = '3.0';
var et_secureId     = 'qPKGYV';
var et_subid        = '';
var et_la           = '';
var et_js           = 1;
var et_co           = '';
...
var et_host		= ('https:' == document.location.protocol ? 'https:' : 'http:' ) + '//application.etracker.com/';
var et_cntHost	= ('https:' == document.location.protocol ? 'https:' : 'http:' ) + '//www.etracker.de/';
var et_et   	= 'qPKGYV';
var et_ibrowse  = 0;
var et_ibrowsev = 99;
var et_ie6	  = 0;
...
function et_iO()
{
	...
	else if((et_location.match(/.et_overlay=1/gi))||(et_d.cookie.match(/et_overlay=1/))||(et_d.cookie.match(/et_overlay=2/)))
	{
		...
		if(et_ie6)
			et_div.innerHTML = '<div id=\"et_div_heatmap\" style=\"visibility:visible;filter:Alpha(opacity=50);width:'+et_px+'px;z-index:1;height:'+(et_py+50)+'px;background-color:#000;\"><img id=\"et_heatmapimage\" style=\"height:'+(et_py+50)+'px;width:1px;visibility:visible;\" src=\"'+et_host+'/face/images/1px.gif\"></div>';
		else
			et_div.innerHTML = '<div id=\"et_div_heatmap\" style=\"filter:Alpha(opacity=50);position:fixed;top:0px;left:0px;visibility:visible;width:100%;height:'+(et_py+50)+'px;background-color:#000;\"></div><img id=\"et_heatmapimage\" style=\"filter:Alpha(opacity=60);position:absolute;top:0px;left:0px;height:'+(et_py+50)+'px;width:1px;visibility:hidden;background-color:#000;\" src=\"'+et_host+'/face/images/1px.gif\">';
...
ET_Event = new etEvent(('https:' == document.location.protocol ? 'https:' : 'http:' ) + '//www.etracker.de', '/eventcnt.php?', 'qPKGYV');
function _etc()
{
	var c = "";
	...
	var s = 0;
	var d = 0.02;
	var u = '8b603c31d58432bef3460cb7d43582e6';
...
var etc_fb_sizes = _etc_fb_get_sizes();
var h = etc_fb_sizes[1];
var w = etc_fb_sizes[0];

function _etc_fb_resize()
{
	var etc_fb_sizes = _etc_fb_get_sizes();
	var h = etc_fb_sizes[1];
	var w = etc_fb_sizes[0]; 
	_etc_fb_cb(('https:' == document.location.protocol ? 'https:' : 'http:' ) + '//visitorvoice.etracker.com/images/fb/fb_button2_br', new Array('','','0px','0px','97px','97px'),new Array('','','0px','0px','97px','97px'), u ? u : '8b603c31d58432bef3460cb7d43582e6', '', '', true); 
	if(document.getElementById("et_vv_fb_content")) 
	{
		if(et_ie6) 
			window.setTimeout("et_set_pos()",300);
		else
			et_set_pos(); 
	}
}

if((h > 485 && w > 475) && !et_blockFB)
{
	...
	var send_prms = '';
	if(((typeof(et_pagename) != 'undefined' && typeof(et_pagename) != 'unknown' && (et_pagename == 'CEWS/Home:home/' || et_pagename == 'CEWS%2FHome%3Ahome%2F' || et_pagename == 'CEWS/Home:home/' || et_pagename == 'CEWS%2FHome%3Ahome%2F')) ||((typeof(et_pagename) == 'undefined' || typeof(et_pagename) == 'unknown') && (document.location.pathname == ('CEWS/Home:home/') ||document.location.pathname == ('CEWS/Home:home/')))) || ((typeof(et_pagename) != 'undefined' && typeof(et_pagename) != 'unknown' && (et_pagename == 'CEWS/Informationsangebote:informationsangebote/' || et_pagename == 'CEWS%2FInformationsangebote%3Ainformationsangebote%2F' || et_pagename == 'CEWS/Informationsangebote:informationsangebote/' || et_pagename == 'CEWS%2FInformationsangebote%3Ainformationsangebote%2F')) ||((typeof(et_pagename) == 'undefined' || typeof(et_pagename) == 'unknown') && (document.location.pathname == ('CEWS/Informationsangebote:informationsangebote/') ||document.location.pathname == ('CEWS/Informationsangebote:informationsangebote/')))) || ((typeof(et_pagename) != 'undefined' && typeof(et_pagename) != 'unknown' && (et_pagename == 'CEWS/Das CEWS:das-cews/' || et_pagename == 'CEWS%2FDas%20CEWS%3Adas-cews%2F' || et_pagename == 'CEWS/Das CEWS:das-cews/' || et_pagename == 'CEWS%2FDas%20CEWS%3Adas-cews%2F')) ||((typeof(et_pagename) == 'undefined' || typeof(et_pagename) == 'unknown') && (document.location.pathname == ('CEWS/Das CEWS:das-cews/') ||document.location.pathname == ('CEWS/Das CEWS:das-cews/')))) || ((typeof(et_pagename) != 'undefined' && typeof(et_pagename) != 'unknown' && (et_pagename == 'GESIS/Datenbanken & Informationssysteme:dienstleistungen/fachinformationen/datenbanken-informationssysteme/' || et_pagename == 'GESIS%2FDatenbanken%20%26%20Informationssysteme%3Adienstleistungen%2Ffachinformationen%2Fdatenbanken-informationssysteme%2F' || et_pagename == 'GESIS/Datenbanken & Informationssysteme:dienstleistungen/fachinformationen/datenbanken-informationssysteme/' || et_pagename == 'GESIS%2FDatenbanken%20%26%20Informationssysteme%3Adienstleistungen%2Ffachinformationen%2Fdatenbanken-informationssysteme%2F')) ||((typeof(et_pagename) == 'undefined' || typeof(et_pagename) == 'unknown') && (document.location.pathname == ('GESIS/Datenbanken & Informationssysteme:dienstleistungen/fachinformationen/datenbanken-informationssysteme/') ||document.location.pathname == ('GESIS/Datenbanken & Informationssysteme:dienstleistungen/fachinformationen/datenbanken-informationssysteme/')))) || ((typeof(et_pagename) != 'undefined' && typeof(et_pagename) != 'unknown' && (et_pagename == 'GESIS/Forschungsdatenbank SOFIS:dienstleistungen/fachinformationen/datenbanken-informationssysteme/forschungsdatenbank-sofis/' || et_pagename == 'GESIS%2FForschungsdatenbank%20SOFIS%3Adienstleistungen%2Ffachinformationen%2Fdatenbanken-informationssysteme%2Fforschungsdatenbank-sofis%2F' || et_pagename == 'GESIS/Forschungsdatenbank SOFIS:dienstleistungen/fachinformationen/datenbanken-informationssysteme/forschungsdatenbank-sofis/' || et_pagename == 'GESIS%2FForschungsdatenbank%20SOFIS%3Adienstleistungen%2Ffachinformationen%2Fdatenbanken-informationssysteme%2Fforschungsdatenbank-sofis%2F')) ||((typeof(et_pagename) == 'undefined' || typeof(et_pagename) == 'unknown') && (document.location.pathname == ('GESIS/Forschungsdatenbank SOFIS:dienstleistungen/fachinformationen/datenbanken-informationssysteme/forschungsdatenbank-sofis/') ||document.location.pathname == ('GESIS/Forschungsdatenbank SOFIS:dienstleistungen/fachinformationen/datenbanken-informationssysteme/forschungsdatenbank-sofis/')))) || ((typeof(et_pagename) != 'undefined' && typeof(et_pagename) != 'unknown' && (et_pagename == 'GESIS/Literaturdatenbank SOLIS:dienstleistungen/fachinformationen/datenbanken-informationssysteme/literaturdatenbank-solis/' || et_pagename == 'GESIS%2FLiteraturdatenbank%20SOLIS%3Adienstleistungen%2Ffachinformationen%2Fdatenbanken-informationssysteme%2Fliteraturdatenbank-solis%2F' || et_pagename == 'GESIS/Literaturdatenbank SOLIS:dienstleistungen/fachinformationen/datenbanken-informationssysteme/literaturdatenbank-solis/' || et_pagename == 'GESIS%2FLiteraturdatenbank%20SOLIS%3Adienstleistungen%2Ffachinformationen%2Fdatenbanken-informationssysteme%2Fliteraturdatenbank-solis%2F')) ||((typeof(et_pagename) == 'undefined' || typeof(et_pagename) == 'unknown') && (document.location.pathname == ('GESIS/Literaturdatenbank SOLIS:dienstleistungen/fachinformationen/datenbanken-informationssysteme/literaturdatenbank-solis/') ||document.location.pathname == ('GESIS/Literaturdatenbank SOLIS:dienstleistungen/fachinformationen/datenbanken-informationssysteme/literaturdatenbank-solis/')))) || ((typeof(et_pagename) != 'undefined' && typeof(et_pagename) != 'unknown' && (et_pagename == 'GESIS/Fachinformation für die Sozialwissenschaften:das-institut/wissenschaftliche-arbeitsbereiche/fachinformation-fuer-die-sozialwissenschaften/' || et_pagename == 'GESIS%2FFachinformation%20f%FCr%20die%20Sozialwissenschaften%3Adas-institut%2Fwissenschaftliche-arbeitsbereiche%2Ffachinformation-fuer-die-sozialwissenschaften%2F' || et_pagename == 'GESIS/Fachinformation fÃ¼r die Sozialwissenschaften:das-institut/wissenschaftliche-arbeitsbereiche/fachinformation-fuer-die-sozialwissenschaften/' || et_pagename == 'GESIS%2FFachinformation%20f%C3%BCr%20die%20Sozialwissenschaften%3Adas-institut%2Fwissenschaftliche-arbeitsbereiche%2Ffachinformation-fuer-die-sozialwissenschaften%2F')) ||((typeof(et_pagename) == 'undefined' || typeof(et_pagename) == 'unknown') && (document.location.pathname == ('GESIS/Fachinformation für die Sozialwissenschaften:das-institut/wissenschaftliche-arbeitsbereiche/fachinformation-fuer-die-sozialwissenschaften/') ||document.location.pathname == ('GESIS/Fachinformation fÃ¼r die Sozialwissenschaften:das-institut/wissenschaftliche-arbeitsbereiche/fachinformation-fuer-die-sozialwissenschaften/')))) || ((typeof(et_pagename) != 'undefined' && typeof(et_pagename) != 'unknown' && (et_pagename == 'GESIS/Fachinformationen:dienstleistungen/fachinformationen/' || et_pagename == 'GESIS%2FFachinformationen%3Adienstleistungen%2Ffachinformationen%2F' || et_pagename == 'GESIS/Fachinformationen:dienstleistungen/fachinformationen/' || et_pagename == 'GESIS%2FFachinformationen%3Adienstleistungen%2Ffachinformationen%2F')) ||((typeof(et_pagename) == 'undefined' || typeof(et_pagename) == 'unknown') && (document.location.pathname == ('GESIS/Fachinformationen:dienstleistungen/fachinformationen/') ||document.location.pathname == ('GESIS/Fachinformationen:dienstleistungen/fachinformationen/')))) || ((typeof(et_pagename) != 'undefined' && typeof(et_pagename) != 'unknown' && (et_pagename == 'GESIS/GESIS-Bibliothekskatalog:dienstleistungen/fachinformationen/datenbanken-informationssysteme/gesis-bibliothekskatalog/' || et_pagename == 'GESIS%2FGESIS-Bibliothekskatalog%3Adienstleistungen%2Ffachinformationen%2Fdatenbanken-informationssysteme%2Fgesis-bibliothekskatalog%2F' || et_pagename == 'GESIS/GESIS-Bibliothekskatalog:dienstleistungen/fachinformationen/datenbanken-informationssysteme/gesis-bibliothekskatalog/' || et_pagename == 'GESIS%2FGESIS-Bibliothekskatalog%3Adienstleistungen%2Ffachinformationen%2Fdatenbanken-informationssysteme%2Fgesis-bibliothekskatalog%2F')) ||((typeof(et_pagename) == 'undefined' || typeof(et_pagename) == 'unknown') && (document.location.pathname == ('GESIS/GESIS-Bibliothekskatalog:dienstleistungen/fachinformationen/datenbanken-informationssysteme/gesis-bibliothekskatalog/') ||document.location.pathname == ('GESIS/GESIS-Bibliothekskatalog:dienstleistungen/fachinformationen/datenbanken-informationssysteme/gesis-bibliothekskatalog/')))) || ((typeof(et_pagename) != 'undefined' && typeof(et_pagename) != 'unknown' && (et_pagename == 'GESIS/SocioGuide:dienstleistungen/fachinformationen/datenbanken-informationssysteme/socioguide/' || et_pagename == 'GESIS%2FSocioGuide%3Adienstleistungen%2Ffachinformationen%2Fdatenbanken-informationssysteme%2Fsocioguide%2F' || et_pagename == 'GESIS/SocioGuide:dienstleistungen/fachinformationen/datenbanken-informationssysteme/socioguide/' || et_pagename == 'GESIS%2FSocioGuide%3Adienstleistungen%2Ffachinformationen%2Fdatenbanken-informationssysteme%2Fsocioguide%2F')) ||((typeof(et_pagename) == 'undefined' || typeof(et_pagename) == 'unknown') && (document.location.pathname == ('GESIS/SocioGuide:dienstleistungen/fachinformationen/datenbanken-informationssysteme/socioguide/') ||document.location.pathname == ('GESIS/SocioGuide:dienstleistungen/fachinformationen/datenbanken-informationssysteme/socioguide/')))) || ((typeof(et_pagename) != 'undefined' && typeof(et_pagename) != 'unknown' && (et_pagename == 'GESIS/Erhebung:dienstleistungen/fachinformationen/datenbanken-informationssysteme/forschungsdatenbank-sofis/erhebung/' || et_pagename == 'GESIS%2FErhebung%3Adienstleistungen%2Ffachinformationen%2Fdatenbanken-informationssysteme%2Fforschungsdatenbank-sofis%2Ferhebung%2F' || et_pagename == 'GESIS/Erhebung:dienstleistungen/fachinformationen/datenbanken-informationssysteme/forschungsdatenbank-sofis/erhebung/' || et_pagename == 'GESIS%2FErhebung%3Adienstleistungen%2Ffachinformationen%2Fdatenbanken-informationssysteme%2Fforschungsdatenbank-sofis%2Ferhebung%2F')) ||((typeof(et_pagename) == 'undefined' || typeof(et_pagename) == 'unknown') && (document.location.pathname == ('GESIS/Erhebung:dienstleistungen/fachinformationen/datenbanken-informationssysteme/forschungsdatenbank-sofis/erhebung/') ||document.location.pathname == ('GESIS/Erhebung:dienstleistungen/fachinformationen/datenbanken-informationssysteme/forschungsdatenbank-sofis/erhebung/')))) || ((typeof(et_pagename) != 'undefined' && typeof(et_pagename) != 'unknown' && (et_pagename == 'GESIS/Themenportale:dienstleistungen/fachinformationen/themenportale/' || et_pagename == 'GESIS%2FThemenportale%3Adienstleistungen%2Ffachinformationen%2Fthemenportale%2F' || et_pagename == 'GESIS/Themenportale:dienstleistungen/fachinformationen/themenportale/' || et_pagename == 'GESIS%2FThemenportale%3Adienstleistungen%2Ffachinformationen%2Fthemenportale%2F')) ||((typeof(et_pagename) == 'undefined' || typeof(et_pagename) == 'unknown') && (document.location.pathname == ('GESIS/Themenportale:dienstleistungen/fachinformationen/themenportale/') ||document.location.pathname == ('GESIS/Themenportale:dienstleistungen/fachinformationen/themenportale/')))) || ((typeof(et_pagename) != 'undefined' && typeof(et_pagename) != 'unknown' && (et_pagename == 'GESIS/Social Sciences in Eastern Europe:dienstleistungen/fachinformationen/themenportale/social-sciences-in-eastern-europe/' || et_pagename == 'GESIS%2FSocial%20Sciences%20in%20Eastern%20Europe%3Adienstleistungen%2Ffachinformationen%2Fthemenportale%2Fsocial-sciences-in-eastern-europe%2F' || et_pagename == 'GESIS/Social Sciences in Eastern Europe:dienstleistungen/fachinformationen/themenportale/social-sciences-in-eastern-europe/' || et_pagename == 'GESIS%2FSocial%20Sciences%20in%20Eastern%20Europe%3Adienstleistungen%2Ffachinformationen%2Fthemenportale%2Fsocial-sciences-in-eastern-europe%2F')) ||((typeof(et_pagename) == 'undefined' || typeof(et_pagename) == 'unknown') && (document.location.pathname == ('GESIS/Social Sciences in Eastern Europe:dienstleistungen/fachinformationen/themenportale/social-sciences-in-eastern-europe/') ||document.location.pathname == ('GESIS/Social Sciences in Eastern Europe:dienstleistungen/fachinformationen/themenportale/social-sciences-in-eastern-europe/')))) || ((typeof(et_pagename) != 'undefined' && typeof(et_pagename) != 'unknown' && (et_pagename == 'CEWS/Home:start/' || et_pagename == 'CEWS%2FHome%3Astart%2F' || et_pagename == 'CEWS/Home:start/' || et_pagename == 'CEWS%2FHome%3Astart%2F')) ||((typeof(et_pagename) == 'undefined' || typeof(et_pagename) == 'unknown') && (document.location.pathname == ('CEWS/Home:start/') ||document.location.pathname == ('CEWS/Home:start/')))))
	_etc_fb_cb(('https:' == document.location.protocol ? 'https:' : 'http:' ) + '//visitorvoice.etracker.com/images/fb/fb_button2_br', new Array('','','0px','0px','97px','97px'),new Array('','','0px','0px','97px','97px'), u ? u : '8b603c31d58432bef3460cb7d43582e6', '', '', false);
	
}	if(((s==5&&(a[2]?a[2]>1348121120:false))||(s==6)||(s==1)||1))
	{
		prms = "et=qPKGYV&u=" + u + "&q=2&t=vst";	
		...
		if(typeof(_etc_ht)=='function')
		{
			try{ prms += "&ref=" + (top.document.referrer ? escape(top.document.referrer) : ''); } catch(e) { prms += "&ref="; };
		}
		c = c + "<img style=\"border:0px;\" src=\""+et_vvHost+"vvcnt.php?" + prms + "\">";
	}
